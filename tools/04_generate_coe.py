#!/usr/bin/env python3
"""
Generate COE files for PCILeech NVMe NE-256 emulation from REAL extracted donor data.
...
Prerequisites:
    - 01_extract_config_space.py (produces config_space_4k.bin)
    - 02_extract_bar0.py (produces nvme_bar0.bin)
    - 03_verify_identify.py (validates identify data)

Usage:
    python 04_generate_coe.py
    python 04_generate_coe.py --output-dir ../75t484_x1/ip
"""
...
DONOR_DIR = os.path.join(SCRIPT_DIR, "donor_data")
DEFAULT_OUTPUT = os.path.join(SCRIPT_DIR, "..", "75t484_x1", "ip")
...
def generate_cfgspace_coe(output_dir):
    """Generate pcileech_cfgspace.coe from real extracted config space binary."""
    bin_path = os.path.join(DONOR_DIR, "config_space_4k.bin")

    if not os.path.exists(bin_path):
        print(f"ERROR: {bin_path} not found. Run 01_extract_config_space.py first.")
        return False
    ...
    json_path = os.path.join(DONOR_DIR, "config_space_parsed.json")
    ...
    write_coe(os.path.join(output_dir, "pcileech_cfgspace.coe"), dwords)
    return True
...
def generate_writemask_coe(output_dir):
    """Generate pcileech_cfgspace_writemask.coe from real config space capability layout."""
    masks = [0x00000000] * 1024  # Default: read-only

    # Load parsed config space to find actual capability offsets
    json_path = os.path.join(DONOR_DIR, "config_space_parsed.json")
    bin_path = os.path.join(DONOR_DIR, "config_space_4k.bin")
    bar_info_path = os.path.join(DONOR_DIR, "bar_info.json")
    ...
    write_coe(os.path.join(output_dir, "pcileech_cfgspace_writemask.coe"), masks)
    return True
...
def generate_bar0_coe(output_dir):
    """Generate pcileech_bar_zero4k.coe from real NVMe BAR0 register dump."""
    dwords = [0] * 1024  # 4KB

    bin_path = os.path.join(DONOR_DIR, "nvme_bar0.bin")
    json_path = os.path.join(DONOR_DIR, "nvme_registers_parsed.json")
    ...
    write_coe(os.path.join(output_dir, "pcileech_bar_zero4k.coe"), dwords)
    return True
...
def generate_identify_coe(output_dir):
    ...
    for name, filename in [("Controller", "identify_controller.bin"),
        ...
        bin_path = os.path.join(DONOR_DIR, filename)
        if not os.path.exists(bin_path):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate NVMe COE files from real donor data")
    parser.add_argument("--output-dir", default=DEFAULT_OUTPUT, help="Output directory for COE files")
    parser.add_argument("--donor-dir", default=DONOR_DIR, help="Donor data directory")
    args = parser.parse_args()

    DONOR_DIR = os.path.abspath(args.donor_dir)
    output_dir = os.path.abspath(args.output_dir)
    os.makedirs(output_dir, exist_ok=True)

    print("=" * 60)
    print("NVMe COE Generator (from REAL donor data)")
    print("=" * 60)
    print(f"Donor data: {DONOR_DIR}")
    print(f"Output:     {output_dir}")

    # Check what data is available
    has_config = os.path.exists(os.path.join(DONOR_DIR, "config_space_4k.bin"))
    has_bar0 = os.path.exists(os.path.join(DONOR_DIR, "nvme_bar0.bin"))
    has_id_ctrl = os.path.exists(os.path.join(DONOR_DIR, "identify_controller.bin"))
    has_id_ns = os.path.exists(os.path.join(DONOR_DIR, "identify_namespace.bin"))

    print(f"\nData availability:")
    print(f"  Config space 4KB: {'YES' if has_config else 'NO -- run 01_extract_config_space.py'}")
    print(f"  BAR0 registers:   {'YES' if has_bar0 else 'NO -- run 02_extract_bar0.py (will use defaults)'}")
    ...
    print("\n[1/4] Config Space (pcileech_cfgspace.coe)")
    results["cfgspace"] = generate_cfgspace_coe(output_dir)

    print("\n[2/4] Write Mask (pcileech_cfgspace_writemask.coe)")
    results["writemask"] = generate_writemask_coe(output_dir)

    print("\n[3/4] BAR0 NVMe Registers (pcileech_bar_zero4k.coe)")
    ...
    if all_ok:
        print("\nAll COE files generated successfully from real donor data.")
    else:
