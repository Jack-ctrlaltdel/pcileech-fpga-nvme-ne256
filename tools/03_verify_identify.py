...
from the real NE-256 donor device) and validates all critical fields.

Usage:
    python 03_verify_identify.py
"""
...
def main():
    ...
    ctrl_path = os.path.join(DONOR_DIR, "identify_controller.bin")
    ...
    ns_path = os.path.join(DONOR_DIR, "identify_namespace.bin")
    ...
    # Save parsed JSON
    ctrl_json_path = os.path.join(DONOR_DIR, "identify_controller_parsed.json")
    with open(ctrl_json_path, "w") as f:
        json.dump(ctrl, f, indent=2, default=str)
    print(f"\nSaved: {ctrl_json_path}")

    ns_json_path = os.path.join(DONOR_DIR, "identify_namespace_parsed.json")
