#!/usr/bin/env python3
"""
Cross-validate all extracted donor data for consistency before firmware build.
...
def load_json(filename):
    """Load JSON from donor_data directory."""
    path = os.path.join(DONOR_DIR, filename)
...
def load_bin(filename):
    """Load binary from donor_data directory."""
    path = os.path.join(DONOR_DIR, filename)
    if not os.path.exists(path):
        return None
...
def main():
    print("=" * 60)
    print("Cross-Validation of Extracted Donor Data")
    print("=" * 60)
